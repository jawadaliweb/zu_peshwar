...
public class SqlPersistenceStorageEngine implements PersistenceStorageEngine {
    ...
    public List<UserWriteRecord> loadUserWrites() {
        ...
        String[] strArr = {CommonProperties.ID, RichPath.TAG_NAME, "type", WRITE_PART_COLUMN_NAME, WRITE_NODE_COLUMN_NAME};
        long currentTimeMillis = System.currentTimeMillis();
    ...
    private void saveWrite(Path path, long j, String str, byte[] bArr) {
        ...
        if (bArr2.length >= 262144) {
            ...
            for (int i = 0; i < splitBytes.size(); i++) {
                ...
                contentValues.put(CommonProperties.ID, Long.valueOf(j));
                contentValues.put(RichPath.TAG_NAME, pathToKey(path));
        ...
        contentValues2.put(CommonProperties.ID, Long.valueOf(j));
        contentValues2.put(RichPath.TAG_NAME, pathToKey(path));
    ...
    public long serverCacheEstimatedSizeInBytes() {
        Cursor rawQuery = this.database.rawQuery(String.format("SELECT sum(length(%s) + length(%s)) FROM %s", new Object[]{"value", RichPath.TAG_NAME, SERVER_CACHE_TABLE}), (String[]) null);
        try {
    ...
    public void saveTrackedQuery(TrackedQuery trackedQuery) {
        ...
        long currentTimeMillis = System.currentTimeMillis();
        ContentValues contentValues = new ContentValues();
        contentValues.put(CommonProperties.ID, Long.valueOf(trackedQuery.id));
        contentValues.put(RichPath.TAG_NAME, pathToKey(trackedQuery.querySpec.getPath()));
    ...
    public List<TrackedQuery> loadTrackedQueries() {
        String[] strArr = {CommonProperties.ID, RichPath.TAG_NAME, TRACKED_QUERY_PARAMS_COLUMN_NAME, TRACKED_QUERY_LAST_USE_COLUMN_NAME, TRACKED_QUERY_COMPLETE_COLUMN_NAME, "active"};
        long currentTimeMillis = System.currentTimeMillis();
        Cursor query = this.database.query(TRACKED_QUERY_TABLE, strArr, (String) null, (String[]) null, (String) null, (String) null, CommonProperties.ID);
        ArrayList arrayList = new ArrayList();
        while (query.moveToNext()) {
            try {
    ...
    public void pruneCache(Path path, PruneForest pruneForest) {
        ...
        if (pruneForest.prunesAnything()) {
            verifyInsideTransaction();
            long currentTimeMillis = System.currentTimeMillis();
            Cursor loadNestedQuery = loadNestedQuery(path2, new String[]{ROW_ID_COLUMN_NAME, RichPath.TAG_NAME});
    ...
    private void saveNode(Path path, Node node) {
        ...
        if (serializeObject.length >= 262144) {
            ...
            for (int i = 0; i < splitBytes.size(); i++) {
                ContentValues contentValues = new ContentValues();
                contentValues.put(RichPath.TAG_NAME, partKey(path, i));
    ...
    private Node loadNested(Path path) {
        ...
        ArrayList arrayList2 = new ArrayList();
        long currentTimeMillis = System.currentTimeMillis();
        Cursor loadNestedQuery = loadNestedQuery(path4, new String[]{RichPath.TAG_NAME, "value"});
        long currentTimeMillis2 = System.currentTimeMillis() - currentTimeMillis;
    ...
    private Cursor loadNestedQuery(Path path, String[] strArr) {
        ...
        return this.database.query(SERVER_CACHE_TABLE, strArr, str, strArr2, (String) null, (String) null, RichPath.TAG_NAME);
    }
