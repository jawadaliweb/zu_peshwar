...
final class Blend {
    ...
    private static float rotationDirection(float f, float f2) {
        ...
        return (abs > abs2 || abs > abs3) ? (abs2 > abs || abs2 > abs3) ? ((double) f5) >= FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE ? 1.0f : -1.0f : ((double) f4) >= FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE ? 1.0f : -1.0f : ((double) f3) >= FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE ? 1.0f : -1.0f;
    }
}
