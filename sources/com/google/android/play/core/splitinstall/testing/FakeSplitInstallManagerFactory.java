...
public class FakeSplitInstallManagerFactory {
    ...
    public static synchronized FakeSplitInstallManager create(Context context, File file) {
        ...
        synchronized (FakeSplitInstallManagerFactory.class) {
            ...
            } else if (!fakeSplitInstallManager2.c().getAbsolutePath().equals(file.getAbsolutePath())) {
                throw new RuntimeException(String.format("Different module directories used to initialize FakeSplitInstallManager: '%s' and '%s'", new Object[]{a.c().getAbsolutePath(), file.getAbsolutePath()}));
            }
