...
public final class TasksKt {
    ...
    private static final <T> Deferred<T> asDeferredImpl(Task<T> task, CancellationTokenSource cancellationTokenSource) {
        ...
        if (task.isComplete()) {
            ...
            task.addOnCompleteListener((Executor) DirectExecutor.INSTANCE, (OnCompleteListener<T>) new OnCompleteListener() {
                public final void onComplete(Task task) {
                    TasksKt.m1612asDeferredImpl$lambda0(CompletableDeferred.this, task);
                }
            });
        }
    ...
    public static final void m1612asDeferredImpl$lambda0(CompletableDeferred completableDeferred, Task task) {
        Exception exception = task.getException();
        if (exception != null) {
