...
public abstract class AbstractLZ77CompressorInputStream extends CompressorInputStream implements InputStreamStatistics {
    ...
    private final CountingInputStream f26in;
    ...
    public AbstractLZ77CompressorInputStream(InputStream inputStream, int i) throws IOException {
        this.f26in = new CountingInputStream(inputStream);
        if (i > 0) {
            this.windowSize = i;
    ...
    public long getCompressedCount() {
        return this.f26in.getBytesRead();
    }
    ...
    private void tryToReadLiteral(int i) throws IOException {
        ...
        int readFully = min > 0 ? IOUtils.readFully(this.f26in, this.buf, this.writeIndex, min) : 0;
        count(readFully);
        if (min == readFully) {
            this.writeIndex += min;
