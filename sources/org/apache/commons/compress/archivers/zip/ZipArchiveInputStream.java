...
public class ZipArchiveInputStream extends ArchiveInputStream implements InputStreamStatistics {
    ...
    public ZipArchiveInputStream(InputStream inputStream, String str, boolean z, boolean z2) {
        ...
        this.f17in = new PushbackInputStream(inputStream, allocate.capacity());
            ...
            if (zipLong3.equals(ZipLong.LFH_SIG)) {
                ...
                if (this.current.entry.getCompressedSize() != -1) {
                    if (!(!ZipUtil.canHandleEntryData(this.current.entry) || methodByCode == ZipMethod.STORED || methodByCode == ZipMethod.DEFLATED)) {
                        BoundedInputStream boundedInputStream = new BoundedInputStream(this.f17in, this.current.entry.getCompressedSize());
                        int i = AnonymousClass1.$SwitchMap$org$apache$commons$compress$archivers$zip$ZipMethod[methodByCode.ordinal()];
                        if (i == 1) {
                            InputStream unused3 = this.current.f19in = new UnshrinkingInputStream(boundedInputStream);
                        } else if (i == 2) {
                            InputStream unused4 = this.current.f19in = new ExplodingInputStream(this.current.entry.getGeneralPurposeBit().getSlidingDictionarySize(), this.current.entry.getGeneralPurposeBit().getNumberOfShannonFanoTrees(), boundedInputStream);
                        } else if (i == 3) {
                            InputStream unused5 = this.current.f19in = new BZip2CompressorInputStream(boundedInputStream);
                        } else if (i == 4) {
                            InputStream unused6 = this.current.f19in = new Deflate64CompressorInputStream((InputStream) boundedInputStream);
                        }
                    }
                } else if (methodByCode == ZipMethod.ENHANCED_DEFLATED) {
                    InputStream unused7 = this.current.f19in = new Deflate64CompressorInputStream(this.f17in);
                }
                this.entriesRead++;
        ...
        if (!this.closed) {
            ...
            if (!supportsDataDescriptorFor(this.current.entry)) {
                ...
                if (this.current.entry.getMethod() == 0) {
                    ...
                    i3 = this.current.f19in.read(bArr, i, i2);
                } else {
    ...
    public long getCompressedCount() {
        ...
        if (this.current.entry.getMethod() == ZipMethod.UNSHRINKING.getCode()) {
            return ((UnshrinkingInputStream) this.current.f19in).getCompressedCount();
        }
        if (this.current.entry.getMethod() == ZipMethod.IMPLODING.getCode()) {
            return ((ExplodingInputStream) this.current.f19in).getCompressedCount();
        }
        if (this.current.entry.getMethod() == ZipMethod.ENHANCED_DEFLATED.getCode()) {
            return ((Deflate64CompressorInputStream) this.current.f19in).getCompressedCount();
        }
        if (this.current.entry.getMethod() == ZipMethod.BZIP2.getCode()) {
            return ((BZip2CompressorInputStream) this.current.f19in).getCompressedCount();
        }
        return -1;
    }
    ...
    private int readStored(byte[] bArr, int i, int i2) throws IOException {
        ...
        if (this.buf.position() >= this.buf.limit()) {
            this.buf.position(0);
            int read = this.f17in.read(this.buf.array());
            if (read != -1) {
                this.buf.limit(read);
                count(read);
                CurrentEntry currentEntry = this.current;
    ...
    private void drainCurrentEntryData() throws IOException {
        ...
        while (compressedSize > 0) {
            long read = (long) this.f17in.read(this.buf.array(), 0, (int) Math.min((long) this.buf.capacity(), compressedSize));
            if (read >= 0) {
                count(read);
                compressedSize -= read;
            } else {
    ...
    private void readFully(byte[] bArr, int i) throws IOException {
        ...
        int readFully = IOUtils.readFully(this.f17in, bArr, i, length);
        count(readFully);
        if (readFully < length) {
    ...
    private void readStoredEntry() throws IOException {
        ...
        int i2 = 0;
        while (!z) {
            int read = this.f17in.read(this.buf.array(), i2, 512 - i2);
            if (read > 0) {
                int i3 = read + i2;
                if (i3 < 4) {
                    i2 = i3;
    ...
    private boolean bufferContainsSignature(java.io.ByteArrayOutputStream r11, int r12, int r13, int r14) throws java.io.IOException {
        /*
            r10 = this;
            r0 = 0
            r1 = 0
        ...
        L_0x0003:
            ...
            java.nio.ByteBuffer r4 = r10.buf
            byte[] r4 = r4.array()
            byte r4 = r4[r2]
            byte[] r5 = LFH
            byte r6 = r5[r0]
            ...
            java.nio.ByteBuffer r8 = r10.buf
            byte[] r8 = r8.array()
            int r9 = r2 + 2
            byte r8 = r8[r9]
            byte r9 = r5[r4]
        ...
        L_0x0048:
            java.nio.ByteBuffer r5 = r10.buf
            byte[] r5 = r5.array()
            byte r5 = r5[r2]
            byte[] r8 = CFH
            byte r9 = r8[r4]
        ...
        L_0x008a:
            ...
            byte[] r5 = r5.array()
            int r3 = r3 - r4
            r10.pushback(r5, r4, r3)
            java.nio.ByteBuffer r3 = r10.buf
            byte[] r3 = r3.array()
            r11.write(r3, r0, r4)
            r10.readDataDescriptor()
        L_0x00a2:
            int r2 = r2 + 1
            goto L_0x0003
        L_0x00a6:
    ...
    private void pushback(byte[] bArr, int i, int i2) throws IOException {
        ((PushbackInputStream) this.f17in).unread(bArr, i, i2);
    ...
    private void realSkip(long j) throws IOException {
        ...
        if (j >= 0) {
            while (j2 < j) {
                long j3 = j - j2;
                InputStream inputStream = this.f17in;
                byte[] bArr = this.skipBuf;
                if (((long) bArr.length) <= j3) {
                    j3 = (long) bArr.length;
                }
    ...
    private class BoundedInputStream extends InputStream {
        ...
        public int read(byte[] bArr, int i, int i2) throws IOException {
            ...
            int read = this.f18in.read(bArr, i, (int) (j >= 0 ? Math.min((long) i2, j - this.pos) : (long) i2));
            if (read == -1) {
                return -1;
            }
            long j2 = (long) read;
        ...
        public long skip(long j) throws IOException {
            ...
            long skip = IOUtils.skip(this.f18in, j);
            this.pos += skip;
            return skip;
        }
