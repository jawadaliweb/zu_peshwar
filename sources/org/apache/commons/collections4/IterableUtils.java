...
public class IterableUtils {
    ...
    public static <O, R extends Collection<O>> List<R> partition(Iterable<? extends O> iterable, Factory<R> factory, Predicate<? super O>... predicateArr) {
        ...
        for (Predicate<? super O> requireNonNull : predicateArr) {
            Objects.requireNonNull(requireNonNull, "Predicate must not be null.");
        }
        ...
        for (Object next : iterable) {
            ...
            while (true) {
                if (i3 >= length) {
                    z = false;
                    break;
                } else if (predicateArr[i3].evaluate(next)) {
                    ((Collection) arrayList.get(i3)).add(next);
                    z = true;
                    break;
                } else {
                    i3++;
            ...
            if (!z) {
                ((Collection) arrayList.get(length)).add(next);
            }
        }
        return arrayList;
    }
