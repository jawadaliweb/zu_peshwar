...
public class FirebaseRemoteConfig {
    ...
    public Task<Void> fetch() {
        return this.fetchHandler.fetch().onSuccessTask(FirebaseExecutors.directExecutor(), $$Lambda$FirebaseRemoteConfig$DICvvb4p9FDGB0VOb0xHLqgDE.INSTANCE);
    }

    public Task<Void> fetch(long j) {
        return this.fetchHandler.fetch(j).onSuccessTask(FirebaseExecutors.directExecutor(), $$Lambda$FirebaseRemoteConfig$JYPoCswQMeSK7ONQc9hg6EWz1E.INSTANCE);
    }
    ...
    private Task<Void> setDefaultsWithStringsMapAsync(Map<String, String> map) {
        try {
            return this.defaultConfigsCache.put(ConfigContainer.newBuilder().replaceConfigsWith(map).build()).onSuccessTask(FirebaseExecutors.directExecutor(), $$Lambda$FirebaseRemoteConfig$Zxj7vj2VFSzqm5ULSomVu6gUxlM.INSTANCE);
